...
public final class Application implements IPathElement, Runnable {
    ...
    */
	    ...
	    for (Iterator i = jobs.iterator (); i.hasNext ();) {
	        ...
	        if (j.appliesToDate (d)) {
	            ...
	            try {
	                ...
	                if (stopped == false) {
	                    logEvent ("couldn't execute " + j + ", maximum thread count reached");
	                    continue;
	                } else {
	                    break;
	                }
	            ...
	            if (j.getTimeout () > 20000 || CronJob.millisToNextFullMinute () < 30000) {
	                CronRunner r = new CronRunner (thisEvaluator, j);
	                r.start ();
	                activeCronJobs.put (j.getName (), r);
	            } else {
	                try {
	                    thisEvaluator.invokeFunction ((INode)null, j.getFunction (), new Object [0], j.getTimeout ());
	                } catch (Exception ex) {
	...
	class CronRunner extends Thread {
	    ...
	    public void run () {
	        try {
	            thisEvaluator.invokeFunction ((INode)null, job.getFunction (), new Object [0], job.getTimeout ());
	        } catch (Exception ex) { }
	        if (stopped == false) {
	            releaseEvaluator (thisEvaluator);
	        }
